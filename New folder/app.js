...
class App {
    constructor() {
        this.games = ['sudoku', 'ball-catch', 'jump', 'run', 'boom', 'music', 'reflex', 'memory', 'avoid', 'snake', 'spectrum', 'animal', 'stack', 'bricks', 'lasers', 'pong', 'floppy', 'hacking', 'catcher', 'mines', 'orbital', 'pulse', 'clicker', 'strike', 'slide', 'mastermind', 'hanoi', 'flood', 'lights', 'math', 'word', 'shape', 'bitmatch', 'mirror', 'rps', 'tictactoe', 'hangman', 'aim', 'dice', 'slots', 'trivia', 'whack', 'typing', 'color'];
        this.pendingGames = [];
        ...
        try {
            this.score = parseInt(sessionStorage.getItem('bored_score')) || 0;
            this.level = parseInt(sessionStorage.getItem('bored_level')) || 1;
            const savedPending = sessionStorage.getItem('bored_pending');
            if (savedPending) {
                this.pendingGames = JSON.parse(savedPending);
            }
        } catch (e) {
        ...
        try {
            const savedLeaderboard = localStorage.getItem('bored_leaderboard');
            if (savedLeaderboard) {
    ...
    updateScore(amount) {
        ...
        sessionStorage.setItem('bored_score', this.score);
    }
    ...
    async loadRandomGame() {
        ...
        sessionStorage.setItem('bored_level', this.level);
        sessionStorage.setItem('bored_pending', JSON.stringify(this.pendingGames));
    ...
    initGame(id) {
        ...
        switch(id) {
            ...
            case 'hangman':
                ...
                break;
            case 'aim':
                game = new AimTrainerGame(this);
                break;
            case 'dice':
                game = new DiceGame(this);
                break;
            case 'slots':
                game = new SlotsGame(this);
                break;
            case 'trivia':
                game = new TriviaGame(this);
                break;
            case 'whack':
                game = new WhackGame(this);
                break;
            case 'typing':
                game = new TypingGame(this);
                break;
            case 'color':
                game = new ColorGuesserGame(this);
                break;
        }
